...
Panel {
  ...
  // Holiday data is cached per year for the life of the shell. The official
  // 2026 schedule is bundled for offline use; other years are fetched on
  // demand from a dataset generated from State Council notices.
  property var holidayYears: ({})
  property var fetchedHolidayYears: ({})
  property var holidayYearQueue: []
  property int holidayRequestYear: 0
  ...
  function goToToday() {
    ...
    Qt.callLater(root.loadVisibleHolidayYears)
  }
  ...
  function setHolidayYear(year, days) {
    var updated = {}
    for (var existing in root.holidayYears) updated[existing] = root.holidayYears[existing]
    updated[String(year)] = days || {}
    root.holidayYears = updated
  }

  function markHolidayYearFetched(year) {
    var updated = {}
    for (var existing in root.fetchedHolidayYears) updated[existing] = root.fetchedHolidayYears[existing]
    updated[String(year)] = true
    root.fetchedHolidayYears = updated
  }

  function loadHolidayYear(year) {
    var requested = Math.round(Number(year))
    if (!isFinite(requested) || requested < 2000 || requested > 2200) return

    var key = String(requested)
    if (!root.holidayYears[key]) root.setHolidayYear(requested, Holidays.bundledYear(requested))
    if (root.fetchedHolidayYears[key] === true) return
    if (holidayFetch.running) {
      if (root.holidayRequestYear === requested || root.holidayYearQueue.indexOf(requested) !== -1) return
      var queued = root.holidayYearQueue.slice()
      queued.push(requested)
      root.holidayYearQueue = queued
      return
    }

    root.startHolidayYearFetch(requested)
  }

  function startHolidayYearFetch(requested) {
    root.holidayRequestYear = requested
    holidayFetch.command = [
      "curl", "-fsS", "--retry", "2", "--connect-timeout", "3", "--max-time", "8",
      "https://cdn.jsdelivr.net/gh/NateScarlet/holiday-cn@master/" + requested + ".json"
    ]
    holidayFetch.running = true
  }

  function startNextHolidayYearFetch() {
    if (holidayFetch.running || root.holidayYearQueue.length === 0) return

    var queued = root.holidayYearQueue.slice()
    var requested = queued.shift()
    root.holidayYearQueue = queued
    if (root.fetchedHolidayYears[String(requested)] === true) {
      Qt.callLater(root.startNextHolidayYearFetch)
      return
    }
    root.startHolidayYearFetch(requested)
  }

  // A fixed six-week grid can contain dates from the previous or next year.
  // Cache every year that is actually visible, rather than only the month
  // named in the heading.
  function loadVisibleHolidayYears() {
    if (!root.weeks || root.weeks.length === 0) return
    var firstWeek = root.weeks[0]
    var lastWeek = root.weeks[root.weeks.length - 1]
    if (!firstWeek.days.length || !lastWeek.days.length) return

    var firstYear = firstWeek.days[0].year
    var lastYear = lastWeek.days[lastWeek.days.length - 1].year
    root.loadHolidayYear(firstYear)
    if (lastYear !== firstYear) root.loadHolidayYear(lastYear)
  }

  function holidayForDate(key) {
    var dateKey = String(key)
  ...
  Process {
    ...
    onExited: Qt.callLater(root.startNextHolidayYearFetch)

    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var parsed = Holidays.parsePayload(text)
        if (parsed.year <= 0 || parsed.year !== root.holidayRequestYear) return
        root.setHolidayYear(parsed.year, parsed.days)
        root.markHolidayYearFetched(parsed.year)
      }
    }
  }
              ...
              Repeater {
                ...
                Row {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      required property var modelData
                      readonly property var holiday: root.holidayForDate(modelData.key)
                      // Official records win over the weekday rule, so a
                      // Saturday/Sunday that is a makeup workday still says
                      // "班". Otherwise regular weekends get a lightweight
                      // "休" badge without the statutory-holiday background.
                      readonly property var dayStatus: holiday || (modelData.weekend
                        ? ({ name: "周末", isOffDay: true, regularWeekend: true })
                        : null)
                      ...
                      color: holiday && holiday.isOffDay
                      ...
                      Text {
                        ...
                        anchors.centerIn: parent
                        text: modelData.day
                        color: parent.holiday
                          ? (parent.holiday.isOffDay
                            ? Color.accent
                            : (modelData.inMonth ? root.contentForeground : Qt.darker(root.contentForeground, 1.45)))
                          : (!modelData.inMonth
                      ...
                      Text {
                        visible: parent.dayStatus !== null
                        anchors.top: parent.top
                        anchors.right: parent.right
                        anchors.topMargin: Style.space(1)
                        anchors.rightMargin: Style.space(3)
                        text: Holidays.marker(parent.dayStatus)
                        color: parent.dayStatus && parent.dayStatus.isOffDay
                          ? Color.accent
